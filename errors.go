...
func (a *Assertion) ErrorIs(expr, target error, msg ...any) *Assertion {
	a.TB().Helper()
	return a.Assert(errors.Is(expr, target), NewFailure("ErrorIs", msg, map[string]any{"err": expr}))
}
...
// Panic 断言函数会发生 panic
func (a *Assertion) Panic(fn func(), msg ...any) *Assertion {
	a.TB().Helper()
	has, _ := hasPanic(fn)
...
func (a *Assertion) PanicString(fn func(), str string, msg ...any) *Assertion {
	a.TB().Helper()

	if has, m := hasPanic(fn); has {
		return a.Assert(strings.Contains(fmt.Sprint(m), str), NewFailure("PanicString", msg, map[string]any{"msg": m}))
	}
	return a.Assert(false, NewFailure("PanicString", msg, nil))
}

// PanicType 断言函数会发生 panic 且抛出指定的类型
func (a *Assertion) PanicType(fn func(), typ any, msg ...any) *Assertion {
	a.TB().Helper()
...
func (a *Assertion) PanicValue(fn func(), v any, msg ...any) *Assertion {
	a.TB().Helper()
...
func (a *Assertion) NotPanic(fn func(), msg ...any) *Assertion {
	a.TB().Helper()
	has, m := hasPanic(fn)
